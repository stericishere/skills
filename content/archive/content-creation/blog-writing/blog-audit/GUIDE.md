...
description: >
  ...
  Spawns parallel teammates via Agent Teams for comprehensive analysis. Produces
  per-post scores and a prioritized action queue. Use when user says "audit blog",
  "blog audit", "site audit", "blog health", "audit all posts", "check all blogs".
allowed-tools:
  - Read
  - Write
  - Bash
...
and AI citation readiness. Uses parallel teammates via Agent Teams for efficient
analysis and produces a prioritized action queue.
...
### Step 2: Parallel Analysis via Agent Teams

Create a team and tasks, then spawn teammates for parallel processing across
all discovered blog files:

```
TeamCreate("blog-audit", "Parallel blog health assessment across all posts")
```

Create a task for each analysis dimension:

```
TaskCreate("content-quality", "Score all posts on 30-point content quality scale")
TaskCreate("seo-optimization", "Check on-page SEO elements for all posts")
TaskCreate("schema-validation", "Validate structured data across all posts")
TaskCreate("link-health", "Map internal links and detect orphan/dead-end pages")
TaskCreate("freshness-check", "Audit content freshness across all posts")
TaskCreate("ai-readiness", "Score AI citation readiness for all posts")
```

Spawn teammates that join the team — each claims a task and works in parallel:

#### Content Quality Teammate
```
Agent(prompt: "Analyze content quality for these blog files: [file list]. Score each post on the 30-point content quality scale. Check paragraph length (target 40-80 words, hard limit 150), sentence length (target 15-20 words), heading structure and question-format headings, readability (Flesch Reading Ease 60-70 target). Return structured scores per post.", team_name: "blog-audit", name: "content-quality")
```
...
#### SEO Optimization Teammate
```
Agent(prompt: "Check on-page SEO elements for these blog files: [file list]. Evaluate title tag length (50-60 chars), meta description (150-160 chars, includes statistic), H1 presence and uniqueness, image alt text coverage, internal and external link counts, URL slug quality. Return structured results per post.", team_name: "blog-audit", name: "seo-optimization")
```
...
#### Schema Validation Teammate
```
Agent(prompt: "Validate structured data for these blog files: [file list]. Detect BlogPosting schema completeness, FAQ schema presence and format, verify dateModified matches lastUpdated frontmatter, flag missing or malformed schema. Return structured results per post.", team_name: "blog-audit", name: "schema-validation")
```
...
#### Link Health Teammate
```
Agent(prompt: "Map internal links for these blog files: [file list]. Build a directed link graph, detect orphan pages (zero inbound internal links), detect dead-end pages (zero outbound internal links), check for broken internal link targets, recommend bidirectional link opportunities. Return the link graph and issues found.", team_name: "blog-audit", name: "link-health")
```
...
#### Freshness Check Teammate
```
Agent(prompt: "Audit content freshness for these blog files: [file list]. Read lastUpdated or dateModified from each post's frontmatter, calculate days since last update, flag posts not updated in 90+ days, categorize by refresh priority. Return freshness data per post.", team_name: "blog-audit", name: "freshness-check")
```
...
#### AI Readiness Teammate
```
Agent(prompt: "Score AI citation readiness for these blog files: [file list]. Check passage-level citability (120-180 word sections), evaluate Q&A formatting and entity clarity, check for TL;DR boxes and citation capsules, assess AI crawler accessibility. Return AI readiness scores per post.", team_name: "blog-audit", name: "ai-readiness")
```
...
Teammates can use `SendMessage` to share cross-cutting findings (e.g., the link
health teammate can notify the SEO teammate about posts with zero internal links).

As each teammate completes, mark its task done:

```
TaskUpdate("content-quality", status: "completed")
TaskUpdate("seo-optimization", status: "completed")
...
```
